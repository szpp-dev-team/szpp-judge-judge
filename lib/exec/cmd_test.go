...
func TestRunCommandSuccess(t *testing.T) {
	res, err := RunCommand("ls >./tmp/exec/1/stdout.txt", "./tmp/exec/1")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunCommandTLE(t *testing.T) {
...
func TestRunCommandCommandError(t *testing.T) {
	res, err := RunCommand("gcc >./tmp/exec/1/stdout.txt 2>./tmp/exec/1/stderr.txt", "./tmp/exec/1")
	if err != nil {
		t.Fatal(err)
	}
	test.AssertNeq(t, res.Stderr, "")
	test.AssertNeq(t, res.ExecutionMemory, 0)
	test.Assert(t, !res.Success)
}
